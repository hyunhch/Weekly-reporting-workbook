...
'Can specify if we're looking for race, gender, or grade
    ...
    MajorArray = Application.Transpose(Range("MajorList"))
    ...
    ElseIf SearchType = "Major" Then
        ReDim SearchArray(0 To UBound(MajorArray))
        ReDim CountArray(0 To UBound(MajorArray))
        SearchArray = MajorArray
...
GradeCount:
    For Each c In SearchHere
        ...
        Else
            CountArray(4) = CountArray(4) + 1 'To catch anything else
        End If
    Next c
...
'Can be called from anywhere
    ...
    Set GradeRange = FindReportRange("<45", "Other Credits")
    Set MajorRange = FindReportRange("Science", "Other Major")
    ...
    TempArray = DemoTabulate(NumChecked, TabulateRange, "Major")
    MajorRange.Offset(PasteRow - 6, 0) = TempArray
    Erase TempArray
