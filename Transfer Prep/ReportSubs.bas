...
'Also called when roster is parsed
    ...
    Set GradeRange = FindReportRange("<45", "Other Credits").Offset(1, 0)
    Set MajorRange = FindReportRange("Science", "Other Major").Offset(1, 0)
    ...
    TempArray = DemoTabulate(TableLength, NameRange, "Major")
    MajorRange = TempArray
    Erase TempArray
...
'Only clears the totals. Called when clearing the roster and clearing the entire report
    ...
    Set ReportSheet = Worksheets("Report Page")
    Set DelRange = FindReportRange("Select", "Other Major")
